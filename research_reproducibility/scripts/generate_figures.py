...
def main() -> None:
    ...
    # F02: measured baseline sensor field.  The 16 half-hour states are summed to hours.
    ...
    baseline_hours = np.loadtxt(ill_path, ndmin=2).sum(axis=1) * 0.5
    ...
    # F07: measured runtime and clearly labelled extrapolation.
    summary = pd.read_json(results / "runtime_summary.json", typ="series")
    values = [float(summary["screening_median_seconds"]), float(summary["hb_median_seconds"]), float(summary["estimated_full_hb_630_seconds"])]
    ...
    ax.set_title("F07. Measured and extrapolated computational cost")
    ax.grid(axis="y", alpha=0.18)
    ...
    stability = pd.read_csv(results / "candidate_rank_stability.csv").head(20).sort_values("top10_frequency")
    fig, ax = plt.subplots(figsize=(8, 6), dpi=320)
    ax.barh(stability["candidate_id"], stability["top10_frequency"], color="#457b9d")
    ax.set_xlim(0, 1)
    ax.set_xlabel("Top-10 selection frequency")
    ax.set_title("F08. Candidate rank robustness under weight perturbation")
    ...
    write_run_metadata(
        ...
        extra={"figure_count": 8, "dpi": 320},
    )

    print(f"Generated 8 paper-support figures in {figures}")
